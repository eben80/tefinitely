...
try {
    ...
    // 2. MRR (Revenue in the last 30 days - Simplified to CAD for this dashboard)
    $stmt = $conn->prepare("SELECT SUM(amount) as mrr FROM subscription_payments WHERE currency = 'CAD' AND payment_date >= DATE_SUB(NOW(), INTERVAL 30 DAY)");
    $stmt->execute();
    $mrr = $stmt->get_result()->fetch_assoc()['mrr'] ?? 0;

    // 3. Revenue Trends (Last 6 months - Simplified to CAD)
    $stmt = $conn->prepare("
