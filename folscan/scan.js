(async function() {
    ...
    const updateDropdown = () => {
        ...
        userList.innerHTML = '<option value="">Select Previous...</option>' + list.map(u => `<option value="${u}">${u}</option>`).join("");
    };
    ...
    const saveUserToHistory = u => {
        ...
        if (!list.includes(u)) { list.unshift(u); localStorage.setItem(storageKey, JSON.stringify(list)); updateDropdown(); }
    };
    ...
    const runScan = async (username) => {
        ...
        const lastFollowers = JSON.parse(localStorage.getItem(`folscan_${username}_followers`) || "[]");
        const lastFollowings = JSON.parse(localStorage.getItem(`folscan_${username}_followings`) || "[]");

        // Logic
        const sections = [
            { title: "🆕 New Followers", list: currentFollowers.filter(f => !lastFollowers.some(l => l.username === f.username)), color: "lightgreen" },
            { title: "❌ Lost Followers", list: lastFollowers.filter(l => !currentFollowers.some(f => f.username === l.username)), color: "salmon" },
            { title: "🆕 New Followings", list: currentFollowings.filter(f => !lastFollowings.some(l => l.username === f.username)), color: "lightblue" },
            { title: "📤 Unfollowed (By You)", list: lastFollowings.filter(l => !currentFollowings.some(f => f.username === l.username)), color: "#ff6b6b" },
            { title: "🚫 Not Following Back", list: currentFollowings.filter(f => !currentFollowers.some(c => c.username === f.username)), color: "orange" },
            { title: "🤝 Mutual", list: currentFollowings.filter(f => currentFollowers.some(c => c.username === f.username)), color: "#00d4ff" }
        ];

        // Sort: Items with lists > 0 come first
        sections.sort((a, b) => (b.list.length > 0) - (a.list.length > 0));

        localStorage.setItem(`folscan_${username}_followers`, JSON.stringify(currentFollowers));
        localStorage.setItem(`folscan_${username}_followings`, JSON.stringify(currentFollowings));

        report.innerHTML = `<h3>📊 Report for @${username}</h3>` + sections.map(s => `
            <details class="folscan-section" ${s.list.length > 0 ? 'open' : ''}>
                <summary style="color:${s.color}">${s.title} (${s.list.length})</summary>
                <div style="padding-left:15px;">
                    ${s.list.map(u => `<a class="folscan-link" href="https://instagram.com/${u.username}" target="_blank">@${u.username} (${u.full_name || 'No Name'})</a>`).join("") || '<em>None</em>'}
                </div>
            </details>`).join("");
    ...
    dlBtn.addEventListener("click", () => {
        const data = { followers: JSON.parse(localStorage.getItem(`folscan_${currentTarget}_followers`)), followings: JSON.parse(localStorage.getItem(`folscan_${currentTarget}_followings`)) };
        const blob = new Blob([JSON.stringify(data, null, 2)], {type: "application/json"});
        const link = document.createElement("a");
        link.href = URL.createObjectURL(blob); link.download = `${currentTarget}_scan.json`; link.click();
    });
    resetBtn.addEventListener("click", () => { if(confirm("Clear history?")) { localStorage.clear(); location.reload(); } });
